...
var mediaPhotoDelete = {
		onDelete: function (element, href, title) {
			...
			$.post("/main-be/doRemoveCampaignSpecialPhoto", { campaignSpecialid: document.getElementById('campaignSpecialid').value, image: photo });
		}
};

$(document).ready(function() {

	$.getJSON('/main-be/getCampaignSpecialMedia/' + document.getElementById('campaignSpecialid').value, function(data) {
		var i;
	...
	$manualEngagementForm = $content.find('#manualEngagement');
		...
		onSuccess: function (data, textStatus, jqXHR) {
			...
			if( obj.response == 'fail') {
				...
				if( document.getElementById('crudtype').value == 'create') {
					window.location = '/main-be/campaignSpecial/' + obj.identifier;
				}
			}
		},
	...
	$.extend( dataTableDefaults, {
		"sAjaxSource": "getCampaignSpecialDeliveredMessagesRecords?campaignSpecial=" + $('#campaignSpecialid').val(),
		"aoColumns" : [
	...
	$('#campaignSpecialDeliveredMessages').dataTable(dataTableDefaults);

	$.extend( dataTableDefaults, {
		"sAjaxSource": "getCampaignSpecialHistoryRecords?campaignSpecialId=" + $('#campaignSpecialId').val(),
		"aoColumns" : [
	...
	$('#campaignSpecialHistory').dataTable(dataTableDefaults);
