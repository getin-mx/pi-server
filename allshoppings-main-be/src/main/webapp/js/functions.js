$(document).ready(function () {
	...
	/*----------------------------------------------------------------------*/
	/* Makes a document as read only
	/*----------------------------------------------------------------------*/
	$.setReadOnly = function() {
		$("input").prop("disabled", true);
		$("select").prop("disabled", true);
		$.each($(".wysiwyg").find("iframe"), function( k, v) {
			console.log(v);
			$(v.contentDocument).keydown(function(event) {
				event.preventDefault();
			});
		});
		$(".comboselectbuttons").children().css("display", "none");
		$(".add").css("display", "none");
		$(".remove").css("display", "none");
		$("#levelBrands").css("display", "none");
		$("#levelShoppings").css("display", "none");
		$("#levelAreas").css("display", "none");

		// This special fields must not be read only
		$("#manualEngagementUserId").prop("disabled", false);
	};
