...
var mediaPhotoDelete = {
		onDelete: function (element, href, title) {
			...
			$.post("/main-be/doRemoveOfferPhoto", { offerid: document.getElementById('offerid').value, image: photo });
		}
};

$(document).ready(function() {

	$.getJSON('/main-be/getOfferMedia/' + document.getElementById('offerid').value, function(data) {
		var i;
		...
		onSuccess: function (data, textStatus, jqXHR) {
			...
			if( obj.response == 'fail') {
				...
				if( document.getElementById('crudtype').value == 'create') {
					window.location = '/main-be/offer/' + obj.identifier;
				}
			}
		},
	...
	$.extend( dataTableDefaults, {
		"sAjaxSource": "getOfferDeliveredMessagesRecords?offer=" + $('#offerid').val(),
		"aoColumns" : [
	...
	$('#offerDeliveredMessages').dataTable(dataTableDefaults);
});
