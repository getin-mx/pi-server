...
var mediaPhotoDelete = {
		onDelete: function (element, href, title) {
			...
			$.post("/main-be/doRemoveBrandPhoto", { brandid: document.getElementById('brandid').value, image: photo });
		}
};

$(document).ready(function() {

	$.getJSON('/main-be/getBrandMedia/' + document.getElementById('brandid').value, function(data) {
	...
	$breadCrumbContent = $content.find('#breadcrumbcontent');
		...
		onSuccess: function (data, textStatus, jqXHR) {
			...
			if( obj.response == 'fail') {
				...
				if( document.getElementById('crudtype').value == 'create') {
					window.location = '/main-be/brand/' + obj.identifier;
				}
			}
		},
