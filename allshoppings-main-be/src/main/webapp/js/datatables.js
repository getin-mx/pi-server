...
(function($, window, document) {
	...
	$.fn.dataTable = function( oInit )
	{
		...
		this.fnSetSettings = function( nNode , settings )
		{
			_fnSetSettingsFromNode( this[_oExt.iApiIndex], settings );
		};
                ...
                ($.inArray('asc', oCol.asSorting) == -1 && $.inArray('desc', oCol.asSorting) == -1) )
      {
		...
		function _fnCalculateColumnWidths ( oSettings )
		{
			...
			else
			{
				...
				for ( i=0 ; i<iColums ; i++ )
				{
					...
					{
						try { nThs[i-iCorrector].style.width = ""; } catch( Exception ) {}
					}
					else
					{
						iCorrector++;
		...
		function _fnSetSettingsFromNode ( nTable, settings )
		{
			for ( var i=0 ; i<_aoSettings.length ; i++ )
			{
				if ( _aoSettings[i].nTable == nTable )
				{
					_aoSettings[i] = settings;
				}
			}
