	...
	var $body = $('body'),
		...
		$form.wl_Form({
			...
			onBeforeSubmit: function(data){
				...
				if(!data.password) {
					myAlert = $.wl_Alert($.i18n._('app.newPassword.missingPassword'), 'info', '#content');
					return false;
				}
				if(!data.passwordConfirmation) {
					myAlert = $.wl_Alert($.i18n._('app.newPassword.missingPasswordConfirmation'), 'info', '#content');
					return false;
				}
				if(data.password != data.passwordConfirmation) {
					myAlert = $.wl_Alert($.i18n._('app.newPassword.confirmationMismatch'), 'info', '#content');
					return false;
				}
				$('.submit').hide();
			},
