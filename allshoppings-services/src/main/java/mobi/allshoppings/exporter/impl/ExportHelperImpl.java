			...
			try {
				...
				while(i.hasNext()) {
					...
					if( dbo.containsField("entityId") && dbo.containsField("mac") && dbo.containsField("checkinType") && dbo.containsField("checkinStarted") && dbo.containsField("checkinFinished")) {
						...
						cnt++;
						if( cnt % 1000 == 0 ) {
							log.log(Level.INFO, "Processing record " + cnt + " of " + total + "...");
						}
						
						String realMac = (String)dbo.get("mac");
						String identifier = Key.fromString((String)dbo.get("_id")).getName();
						String mac = new String(realMac);

						if( true == unit.getHideMac() ) {
						...
						Integer entityKind = (Integer)dbo.get("entityKind");
						String devicePlatform = (String)dbo.get("devicePlatform");
						...
						Date objLastUpdate = (Date)dbo.get("lastUpdate");
