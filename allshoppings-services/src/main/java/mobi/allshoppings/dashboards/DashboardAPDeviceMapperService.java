...
//				if( CollectionUtils.isEmpty(phases) || phases.contains(PHASE_APDEVICE))
//					createGetinAPDevicePerformanceDashboardForDay(curDate, entityIds);
...
//			}
	...
	// GetIn Antennas performance -------------------------------------------------------------------------------------------------------------------------

	public void createGetinAPDevicePerformanceDashboardForDay(Date date, List<String> entityIds) throws ASException {

//		log.log(Level.INFO, "Starting to create APDevice Performance Dashboard for Day " + date + "...");
//		long startTime = new Date().getTime();
//
//		Date processDate = DateUtils.truncate(date, Calendar.DAY_OF_MONTH);
//		Date limitDate = DateUtils.addDays(processDate, 1);
...
//		try {
//
//			// Prepares the external DB Query
//			Date dateFrom = new Date(date.getTime());
//			Date dateTo = new Date(dateFrom.getTime() + 86400000);
//
//			// Looks for all visit records
...
//					+ "cast(convert_tz(quit_at,'UTC','America/Mexico_City') AS datetime) as quit_at, "
//					+ "possible_fake "
//					+ "from visit where "
//					+ "cast(convert_tz(enter_at,'UTC','America/Mexico_City') AS datetime) >= timestamp('" + sdf.format(dateFrom) + "') and "
//					+ "cast(convert_tz(enter_at,'UTC','America/Mexico_City') AS datetime) <= timestamp('" + sdf.format(dateTo) + "')");
//			if( rs.first()) {
//				while(!rs.isAfterLast()) {
//					int storeId = rs.getInt("id_store");
//					String mac = rs.getString("mac");
//					Date enterAt = rs.getTimestamp("enter_at");
//					Date quitAt = rs.getTimestamp("quit_at");
//					int possibleFake = rs.getInt("possible_fake");
//					String deviceType = getDeviceType(mac);
//
//					try {
//						Store store = storeCache.get(String.valueOf(storeId));
//						if( store != null ) {
//							if( CollectionUtils.isEmpty(entityIds) || entityIds.contains(store.getBrandId())) {
//								DashboardIndicatorData obj;
//
//								// visitor_total_records --------------------------------------------------------------------------------
//								// ------------------------------------------------------------------------------------------------------
...
//									if(indicatorsSet.containsKey(obj.getKey().getName())) 
//										obj = indicatorsSet.get(obj.getKey().getName());
//									obj.setDoubleValue(obj.getDoubleValue() + calculateDiffTime(quitAt, enterAt));
//									obj.setRecordCount(obj.getRecordCount() + 1);
//									indicatorsSet.put(obj.getKey().getName(), obj);
//
//									// Device selector
...
//
//			long endTime = new Date().getTime();
//			log.log(Level.INFO, "Finished to create APDevice Performance Dashboard for Day " + date + " in " + (endTime - startTime) + "ms");
//
//		} catch( Exception e ) {
//			log.log(Level.SEVERE, e.getMessage(), e);
//			throw ASExceptionHelper.defaultException(e.getLocalizedMessage(), e);
