...
import mobi.allshoppings.dao.QuarterlyDashboardIndicatorDataDAO;
...
public class QuarterlyDashboardIndicatorDataDAOJDOImpl extends GenericDAOJDO<QuarterlyDashboardIndicatorData> implements QuarterlyDashboardIndicatorDataDAO {
	
	@SuppressWarnings("unused")
	private static final Logger log = Logger.getLogger(QuarterlyDashboardIndicatorDataDAOJDOImpl.class.getName());

	private static final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	
	public QuarterlyDashboardIndicatorDataDAOJDOImpl() {
		super(QuarterlyDashboardIndicatorData.class);
	}

	@Override
	public Key createKey(QuarterlyDashboardIndicatorData obj) throws ASException {
		return keyHelper.obtainKey(QuarterlyDashboardIndicatorData.class,
				String.valueOf(obj.hashCode()) +"-" +UUID.randomUUID());
	}

	@Override
	public List<QuarterlyDashboardIndicatorData> getUsingFilters(String entityId,
	...
	@Override
	public List<QuarterlyDashboardIndicatorData> getUsingFilters(List<String> entityId,
			Integer entityKind, List<String> elementId, List<String> elementSubId,
		...
		List<QuarterlyDashboardIndicatorData> returnedObjs = CollectionFactory.createList();
		...
		try {
			Query query = pm.newQuery(QuarterlyDashboardIndicatorData.class);
			...
			List<QuarterlyDashboardIndicatorData> objs = parameters.size() > 0
					? (List<QuarterlyDashboardIndicatorData>) query.executeWithMap(parameters)
					: (List<QuarterlyDashboardIndicatorData>) query.execute();
			if (objs != null) {
				// force to read
				for (QuarterlyDashboardIndicatorData obj : objs) {
	...
	public void deleteUsingSubentityIdAndElementIdAndDateAndTimezoneOffset(String subentityId, List<String> elementId,
		...
		try {
			...
			db.getCollection("QuarterlyDashboardIndicatorData").remove(query);
			jdoConn.close();
			
			pm.evictAll(true, QuarterlyDashboardIndicatorData.class);
