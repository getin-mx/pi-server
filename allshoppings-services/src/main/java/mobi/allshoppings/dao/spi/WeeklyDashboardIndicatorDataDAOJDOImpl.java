...
public class WeeklyDashboardIndicatorDataDAOJDOImpl extends GenericDAOJDO<WeeklyDashboardIndicatorData> implements WeeklyDashboardIndicatorDataDAO {
	
	@SuppressWarnings("unused")
	private static final Logger log = Logger.getLogger(WeeklyDashboardIndicatorDataDAOJDOImpl.class.getName());

	private static final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	
	public WeeklyDashboardIndicatorDataDAOJDOImpl() {
		super(WeeklyDashboardIndicatorData.class);
	}

	@Override
	public Key createKey(WeeklyDashboardIndicatorData obj) throws ASException {
		return keyHelper.obtainKey(WeeklyDashboardIndicatorData.class,
				String.valueOf(obj.hashCode()) +"-" +UUID.randomUUID());
	}

	@Override
	public List<WeeklyDashboardIndicatorData> getUsingFilters(String entityId,
	...
	@Override
	public List<WeeklyDashboardIndicatorData> getUsingFilters(List<String> entityId,
			Integer entityKind, List<String> elementId, List<String> elementSubId,
		...
		List<WeeklyDashboardIndicatorData> returnedObjs = CollectionFactory.createList();
		...
		try {
			Query query = pm.newQuery(WeeklyDashboardIndicatorData.class);
			...
			List<WeeklyDashboardIndicatorData> objs = parameters.size() > 0
					? (List<WeeklyDashboardIndicatorData>) query.executeWithMap(parameters)
					: (List<WeeklyDashboardIndicatorData>) query.execute();
			if (objs != null) {
				// force to read
				for (WeeklyDashboardIndicatorData obj : objs) {
	...
	public void deleteUsingSubentityIdAndElementIdAndDateAndTimezoneOffset(String subentityId, List<String> elementId,
		...
		try {
			...
			db.getCollection("WeeklyDashboardIndicatorData").remove(query);
			jdoConn.close();
			
			pm.evictAll(true, WeeklyDashboardIndicatorData.class);
