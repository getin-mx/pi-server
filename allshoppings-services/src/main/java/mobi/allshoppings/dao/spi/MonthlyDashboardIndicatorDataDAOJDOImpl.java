...
import mobi.allshoppings.dao.MonthlyDashboardIndicatorDataDAO;
...
public class MonthlyDashboardIndicatorDataDAOJDOImpl extends GenericDAOJDO<MonthlyDashboardIndicatorData> implements MonthlyDashboardIndicatorDataDAO {
	
	@SuppressWarnings("unused")
	private static final Logger log = Logger.getLogger(MonthlyDashboardIndicatorDataDAOJDOImpl.class.getName());

	private static final SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	
	public MonthlyDashboardIndicatorDataDAOJDOImpl() {
		super(MonthlyDashboardIndicatorData.class);
	}

	@Override
	public Key createKey(MonthlyDashboardIndicatorData obj) throws ASException {
		return keyHelper.obtainKey(MonthlyDashboardIndicatorData.class,
				String.valueOf(obj.hashCode()) +"-" +UUID.randomUUID());
	}

	@Override
	public List<MonthlyDashboardIndicatorData> getUsingFilters(String entityId,
	...
	@Override
	public List<MonthlyDashboardIndicatorData> getUsingFilters(List<String> entityId,
			Integer entityKind, List<String> elementId, List<String> elementSubId,
		...
		List<MonthlyDashboardIndicatorData> returnedObjs = CollectionFactory.createList();
		...
		try {
			Query query = pm.newQuery(MonthlyDashboardIndicatorData.class);
			...
			List<MonthlyDashboardIndicatorData> objs = parameters.size() > 0
					? (List<MonthlyDashboardIndicatorData>) query.executeWithMap(parameters)
					: (List<MonthlyDashboardIndicatorData>) query.execute();
			if (objs != null) {
				// force to read
				for (MonthlyDashboardIndicatorData obj : objs) {
	...
	public void deleteUsingSubentityIdAndElementIdAndDateAndTimezoneOffset(String subentityId, List<String> elementId,
		...
		try {
			...
			db.getCollection("MonthlyDashboardIndicatorData").remove(query);
			jdoConn.close();
			
			pm.evictAll(true, MonthlyDashboardIndicatorData.class);
