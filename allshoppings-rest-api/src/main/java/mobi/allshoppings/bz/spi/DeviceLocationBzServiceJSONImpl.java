 ...
 *
 */
public class DeviceLocationBzServiceJSONImpl
extends RestBaseServerResource
implements DeviceLocationBzService {

	private static final Logger log = Logger.getLogger(DeviceLocationBzServiceJSONImpl.class.getName());
			...
			if( deviceLocation.getLat() != 0 && deviceLocation.getLon() != 0 ) {
				...
				if( !systemConfiguration.isTrackOnlyValidZones() || isInValidZone ) {
					if( updateLimitOverpassed || dist > 100 || ( StringUtils.hasText(userId) && !userId.equals(deviceLocation.getUserId()))) {
						...
						DeviceInfo di = null;
						if( !systemConfiguration.isTrackOnlyValidDevices() || validDevices.contains(deviceLocation.getDeviceUUID())) {
							...
							if( userId == null ) {
								...
								if( di != null && !userId.equals(di.getUserId())) {
									di.setUserId(userId);
									needsToUpdateDeviceInfo = true;
								}
							}
				...
				if( deviceLocation.getCountry() != null ) {	
					...
					List<HotSpotAdapter> bs = deserializeHotSpotList((String)cacheHelper.get(cacheKey));
					if( bs != null ) {
						...
						List<Shopping> hotSpots = shoppingDao.getUsingIdList(geoIds);
						...
						List<Store> hotSpots2 = storeDao.getUsingIdList(geoIds);
						...
						try {
							cacheHelper.put(cacheKey , serializeHotSpotList(adapter.getNearSpots()));
						} catch( Exception e ) {}
					}
				}
				...
				// Hour is defined in GMT... so for Mexico we rest 5 hours
				hour = (hour < 5) ? (hour + 24 - 5) : (hour - 5);
				if( hour >= 9 ) {
