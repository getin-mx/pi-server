 ...
 *
 */
public class DeviceInfoBzServiceJSONImpl
        extends RestBaseServerResource
        implements DeviceInfoBzService {

    private static final Logger log = Logger.getLogger(DeviceInfoBzServiceJSONImpl.class.getName());
            ...
            if( userId != null ) {
            	try {
            		...
            		if( user != null && user.getSecuritySettings().getRole().equals(Role.APPLICATION)) {
            			...
            			if( newUser ) {
            				userDao.create(appInternalUser);
            			} else {
            				userDao.updateWithoutChangingMail(appInternalUser);
            			}
