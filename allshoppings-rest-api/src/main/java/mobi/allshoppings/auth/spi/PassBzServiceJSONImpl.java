...
import mobi.allshoppings.auth.PassBzService;
...
public class PassBzServiceJSONImpl  extends RestBaseServerResource implements PassBzService {
	private static final Logger log = Logger.getLogger(PassBzServiceJSONImpl.class.getName());
	...
	public String change(JsonRepresentation entity) {
		...
		try{
			...
			if(this.hasDefaultParameters(obj, DEFAULT_FIELDS) == false){
				return this.getJSONRepresentationFromException(ASExceptionHelper.invalidArgumentsException()).toString();
			}
	...
	public String force(JsonRepresentation entity) {
		...
		try{
			...
			if(this.hasDefaultParameters(obj, SET_FIELDS) == false){
				return this.getJSONRepresentationFromException(ASExceptionHelper.invalidArgumentsException()).toString();
			}
